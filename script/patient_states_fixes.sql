...
INSERT INTO temp_patient_list (patient_program_id, patient_id)
	...
	WHERE voided = 0 AND location_id = (SELECT property_value FROM openmrs_production.global_property WHERE property = "current_health_center_id")
...
DELETE FROM openmrs_production.patient_state
	WHERE state = 1 ;

DELETE FROM openmrs_production.patient_state
	WHERE state IN (6,2)  AND start_date <= '2012-02-12' ;

UPDATE openmrs_production.person SET death_date = NULL, dead = 0 
	WHERE date_created <= '2012-02-12' ;

DELETE FROM openmrs_production.patient_state
	WHERE state = 7;

INSERT INTO openmrs_production.patient_program (patient_id, program_id, date_enrolled, 
            creator, date_created, uuid, location_id)
	SELECT patient_id, 1, NOW(), creator, 
    		date_created, (SELECT UUID()) AS uuid, 
    		(SELECT property_value FROM openmrs_production.global_property WHERE property = "current_health_center_id")
	FROM openmrs_production.patient 
	WHERE voided = 0 AND patient_id NOT IN (SELECT patient_id FROM patient_program WHERE program_id = 1 AND voided = 0); 

INSERT INTO openmrs_production.patient_state (patient_program_id, state, start_date, creator, date_created, uuid)
    SELECT pp.patient_program_id, 1, pp.date_enrolled, 1, NOW(), (SELECT UUID())
    FROM  openmrs_production.patient p
        LEFT JOIN openmrs_production.patient_program pp ON p.patient_id = pp.patient_id AND pp.program_id = 1 AND pp.voided = 0
    WHERE p.voided = 0;
...
FROM openmrs_production.patient_program pp
INNER JOIN (SELECT obs.person_id, MIN(obs.obs_datetime) AS obs_datetime FROM openmrs_production.drug_order d
    LEFT JOIN openmrs_production.orders o ON d.order_id = o.order_id
    LEFT JOIN openmrs_production.obs ON d.order_id = obs.order_id
...
UPDATE openmrs_production.patient_program
    SET date_enrolled = (SELECT MIN(start_date)
                            FROM openmrs_production.patient_state ps2 
                            WHERE ps2.patient_program_id = openmrs_production.patient_program.patient_program_id)
    WHERE program_id = 1;

UPDATE openmrs_production.patient_state 
    SET start_date = (SELECT date_enrolled
                    FROM openmrs_production.patient_program pp2 
                    WHERE pp2.patient_program_id = openmrs_production.patient_state.patient_program_id)
    WHERE state = 1;
...
UPDATE openmrs_production.patient_state 
    SET end_date = (SELECT start_date
                    ...
                    WHERE pp.patient_program_id = openmrs_production.patient_state.patient_program_id)
    WHERE state = 1;
...
INSERT INTO openmrs_production.patient_state (patient_program_id, state, start_date, creator, date_created, uuid)
    SELECT pp.patient_program_id, 3, MIN(o.obs_datetime), 1, NOW(), (SELECT UUID())
        FROM mpc_bart1.obs o LEFT JOIN openmrs_production.patient_program pp ON o.patient_id = pp.patient_id AND pp.program_id = 1
...
UPDATE openmrs_production.patient_program
    SET date_completed = (SELECT MIN(start_date)
                            FROM temp_patient_list pp
                            WHERE pp.patient_program_id = openmrs_production.patient_program.patient_program_id
                            GROUP BY pp.patient_program_id)
...
SELECT t.patient_program_id, 7, MIN(DATE(obs1.obs_datetime)) AS dispensation_date, 1, ADDTIME(NOW(), -2000), (SELECT UUID())
    FROM temp_patient_list t
        LEFT JOIN (SELECT obs.person_id, DATE(obs.obs_datetime) AS obs_datetime 
                        FROM openmrs_production.drug_order d 
                            LEFT JOIN openmrs_production.orders o ON d.order_id = o.order_id
                            LEFT JOIN openmrs_production.obs ON d.order_id = obs.order_id
                        WHERE d.drug_inventory_id IN (SELECT drug_id FROM drug WHERE concept_id IN (SELECT concept_id FROM concept_set WHERE concept_set = 1085)) 
                            AND quantity > 0
