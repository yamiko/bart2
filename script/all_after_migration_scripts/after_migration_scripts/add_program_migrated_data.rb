...
Encounter.find_by_sql("
				...
				WHERE encounter_type = (SELECT encounter_type_id FROM encounter_type WHERE name = 'hiv staging')
				AND patient_id not IN (SELECT DISTINCT(patient_id) FROM patient_program WHERE program_id = 1)").each {|patient |
				current = Patient.find(patient.patient_id)
				
				if current.patient_programs.in_programs("HIV PROGRAM").blank?
