...
def update_outcome(patient, outcome_date)
  ...
  current_active_state = PatientState.create(:patient_program_id => patient_program.id,
    :state => 2, :start_date => outcome_date.to_date)
...
=end
  ...
  reason_obs['value_coded_or_text'] = 'Patient transferred out'
  ...
  reason_obs[:obs_datetime] = encounter.encounter_datetime || Time.now()
  reason_obs[:person_id] ||= encounter.patient_id
  reason_obs['value_numeric'] = current_active_state.id
  Observation.create(reason_obs)
  ...
  if current_active_state.name.upcase == 'PATIENT TRANSFERRED OUT'
    ...
    observation[:obs_datetime] = outcome_date
    observation[:person_id] ||= encounter.patient_id
    observation['value_numeric'] = UnknownLocation.id
