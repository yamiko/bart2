    ...
    def self.lab_trail(patient_identifier,test_types)
      sample_ids_and_test_dates = LabSample.find_by_sql(["SELECT * FROM Lab_Sample join Lab_Parameter,LabTestTable where LabTestTable.Pat_ID IN (?) and Lab_Parameter.sample_id=Lab_Sample.sample_id and LabTestTable.AccessionNum = Lab_Sample.AccessionNum and Lab_Parameter.testtype IN (?)",patient_identifier,test_types]).collect{|sample|[sample.Sample_ID,sample.DATE]} rescue nil
      return if sample_ids_and_test_dates.blank?
