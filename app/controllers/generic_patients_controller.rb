...
#=end
  ...
  def search_all
    ...
    unless search_by_identifier
      ...
      patients = PersonName.find(:all ,:joins => [:person], :conditions => ["person.voided = 0 AND family_name LIKE ? AND given_name LIKE ?",
...
EOF
      ...
      color = 'blue'
      patients.each do |person_name|
        next if person_name.person.addresses.blank?
        next if person_name.person.patient.blank?
        if color == 'blue'
          color = 'white'
        else
          color='blue'
        end
        bean = PatientService.get_patient(person_name.person)
        @html+= <<EOF
<tr>
  <td class='color_#{color} patient_#{person_name.person.id}' style="text-align:left;" onclick="setPatient('#{person_name.person.id}','#{color}','#{side}')">Name:&nbsp;#{bean.name || '&nbsp;'}</td>
  <td class='color_#{color} patient_#{person_name.person.id}' style="text-align:left;" onclick="setPatient('#{person_name.person.id}','#{color}','#{side}')">Age:&nbsp;#{bean.age || '&nbsp;'}</td>
</tr>
<tr>
  <td class='color_#{color} patient_#{person_name.person.id}' style="text-align:left;" onclick="setPatient('#{person_name.person.id}','#{color}','#{side}')">Guardian:&nbsp;#{bean.guardian rescue '&nbsp;'}</td>
  <td class='color_#{color} patient_#{person_name.person.id}' style="text-align:left;" onclick="setPatient('#{person_name.person.id}','#{color}','#{side}')">ARV number:&nbsp;#{bean.arv_number rescue '&nbsp;'}</td>
</tr>
<tr>
  <td class='color_#{color} patient_#{person_name.person.id}' style="text-align:left;" onclick="setPatient('#{person_name.person.id}','#{color}','#{side}')">National ID:&nbsp;#{bean.national_id rescue '&nbsp;'}</td>
  <td class='color_#{color} patient_#{person_name.person.id}' style="text-align:left;" onclick="setPatient('#{person_name.person.id}','#{color}','#{side}')">TA:&nbsp;#{bean.home_district rescue '&nbsp;'}</td>
</tr>
EOF
      end
