...
=end
  ...
  def handle_appointments(visit_date, number_of_days_to_add ,holidays, booked_dates, clinic_days)
  
    number_of_suggested_booked_dates_tried = 0
    date = visit_date.to_date
    
    suggest_date = date + number_of_days_to_add.to_i.days
    ...
    while skip
      ...
      clinic_days.each do |d|
        if (d.to_s.upcase == suggest_date.strftime('%A').to_s.upcase)
          skip = is_holiday(suggest_date, holidays);
          break
        end
      end

      if (skip)
        suggest_date = suggest_date - 1.day
      end
      ...
      if test
        return return_original_suggested_date(suggest_date, booked_dates)
      end

      if (skip == false)
        below_limit = is_below_limit(suggest_date, booked_dates)
