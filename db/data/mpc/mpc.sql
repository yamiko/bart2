
UPDATE `global_property` SET `property_value` = '700' WHERE `property` = 'current_health_center_id';
UPDATE `global_property` SET `property_value` = 'MARTIN PREUSS CENTRE' WHERE `property` = 'current_health_center_name';
UPDATE `global_property` SET `property_value` = 'yes' WHERE `property` = 'use.user.selected.activities';
UPDATE `global_property` SET `property_value` = 'yes' WHERE `property` = 'use.filing.number';
UPDATE `global_property` SET `property_value` = 'yes' WHERE `property` = 'use.extended.staging.questions';
UPDATE `global_property` SET `property_value` = 'true' WHERE `property` = 'simple_application_dashboard';
UPDATE `global_property` SET `property_value` = 'yes' WHERE `property` = 'demographics.middle_name';
UPDATE `global_property` SET `property_value` = 'yes' WHERE `property` = 'demographics.visit_home_for_treatment';
UPDATE `global_property` SET `property_value` = 'yes' WHERE `property` = 'demographics.sms_for_TB_therapy';
UPDATE `global_property` SET `property_value` = 'yes' WHERE `property` = 'demographics.ground_phone';






